...
namespace RadicalMotor.Controllers
{
    ...
    public class AccountsApiController : ControllerBase
    {
        ...
        public AccountsApiController(IAccountRepository accountRepository, ICustomerRepository customerRepository)
        {
        ...
        public async Task<ActionResult<Account>> PostAccount([FromBody] AccountCreateDTO accountDto)
        {
            ...
            var existingAccount = await _accountRepository.GetAccountAsync(accountDto.Username);
            if (existingAccount != null)
            {
                return Conflict(new { message = "An account with the given username already exists." });
            }

            var defaultAccountType = await _accountRepository.GetDefaultAccountTypeAsync("Member");
            if (defaultAccountType == null)
            {
                return BadRequest("Default account type not found.");
            }

            var createdAccount = await _accountRepository.CreateAccountAsync(accountDto.Username, accountDto.Email, accountDto.Password);
        ...
        [HttpPut("{accountId}")]
        public async Task<IActionResult> PutAccount(string accountId, [FromBody] AccountDTO accountDto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (!string.IsNullOrEmpty(accountDto.Username) && accountDto.Username != account.Username)
            {
                account.Username = accountDto.Username;
            }

            if (!string.IsNullOrEmpty(accountDto.Email) && accountDto.Email != account.Email)
            {
                account.Email = accountDto.Email;
            }

            if (!string.IsNullOrEmpty(accountDto.PhoneNumber) && accountDto.PhoneNumber != account.PhoneNumber)
            {
